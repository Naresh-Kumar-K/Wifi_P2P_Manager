...
import com.example.wifip2p.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity() {
    ...
    var peerListListener: WifiP2pManager.PeerListListener = WifiP2pManager.PeerListListener { peerList ->
        //if (peerList != null) {
        val refreshedPeer = peerList.deviceList
        if (refreshedPeer != peers) {
            peers.clear()
            peers.addAll(refreshedPeer)
            ...
            for (device in peerList.deviceList) {
                ...
                dvcArr += device
                // dvcNameArr.add(device.deviceName + "\n" + device.deviceAddress)
                Log.d("NK???",device.deviceName.toString())
            }

            val adapter: ArrayAdapter<String> = ArrayAdapter<String>(
                this,
                android.R.layout.simple_list_item_1, dvcNameArr)
    ...
    var handler: Handler = Handler { msg ->
        when (msg.what) {
            1 -> {
                ...
                binding.readMsg.text = String(readBuff, 0, msg.arg1)
            }
        }
        true
    }
    ...
    private fun init() {
        ...
        val wifiSwitch = binding.wifiSwitch
        wifiSwitch.setOnClickListener {
            switch()
        }
        discover()

        val send: Button = binding.sendButton

        send.setOnClickListener {
            val msg: String = binding.writeMsg.text.toString()
    ...
    private fun discover() {
        ...
        connState = binding.connectionStatus

        val discover = binding.discover
    ...
    private fun updateDvcList() {
        ...
        this.binding.peerListView.setOnItemClickListener { _, _, i, _ ->
